...
The same algorithm implemented in Python is:
...
'''
class SortedList[T:int|float](list[T]):
    def __init__(self, items:list[T], already_sorted:bool=False):
        sorted_items = items if already_sorted else sorted(items)
